...
export default function ActivitiesPage() {
    ...
    const fetchFilter = async () => {
        try {
            const response = await axios.post(`/api/post/search-post?pageNumber=${page}`, filters);
    ...
    const fetchActivities = async (pageNum = 1, reset = false) => {
        try {
            ...
            const response = await axios.get(`/api/post/get-all-posts-for-user?pageNumber=${pageNum}`);
