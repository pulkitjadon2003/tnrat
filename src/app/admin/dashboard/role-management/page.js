...
export default function RoleManagement() {
    ...
    const fetchSubAdmin = async () => {
        try {
            ...
            const res = await axios.get(`/api/super-admin/get-all-sub-admin?pageNumber=${currentPage}`);

            const result = res.data;

            if (result.status === true) {
                setSubAdminList(result?.subAdmin || []);
                setTotalPages(result?.totalPages || 0);
                setTotalSubAdmins(result?.totalSubAdmin || 0);
            } else {
                console.error('Failed to fetch sub-admins:', result);
            }
        } catch (error) {
    ...
    const handleSearch = async (pageNumber = currentPage) => {
        ...
        try {
            const result = await axios.post(`/api/super-admin/search-sub-admin?pageNumber=${pageNumber}`, { search: searchTerm });

            if (result.data?.status === true) {
                setSubAdminList(result?.data?.subAdmin || []);
                setTotalPages(result?.data?.totalSubAdmin || 0);
            } else {
    ...
    const handleDelete = async () => {
        try {
            ...
            const response = await axios.delete(`/api/super-admin/delete-sub-admin?id=${selectedData?._id}`);
            ...
            if (result.status === true) {
                ...
                console.error('Failed to delete sub-admin:', result);
            }
        } catch (error) {
        ...
        return (
            <div className="flex flex-wrap gap-1">
                {permissions?.map((permission, index) => (
                    <span
                        key={index}
                        className="px-2 py-1 text-xs bg-blue-100 text-blue-800 border border-blue-200 rounded-full"
                    >
                        {permission}
                    </span>
                ))}
    ...
    if (loading && subAdminList.length === 0) {
        return (
    ...
    const totalPagesCount = Math.ceil(totalSubAdmins / itemsPerPage);
                ...
                <div className="p-4 border-b border-gray-200">
                    <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                        <div>
                            ...
                            <p className="text-sm text-gray-600">Total {totalSubAdmins} Sub Admins found</p>
                        </div>
                            ...
                            {subAdminList?.length > 0 ? (
                                subAdminList?.map((subAdmin) => (
                                    ...
                                    >
                                        ...
                                        <td className="px-4 py-3 text-sm text-gray-900">{subAdmin?.email}</td>
                                        <td className="px-4 py-3 text-sm text-gray-900">{subAdmin?.phone}</td>
                                        <td className="px-4 py-3">
                                            <PermissionTags permissions={subAdmin?.permission} />
                                        </td>
                                        <td>
                                ...
                                <tr>
                                    <td colSpan="5" className="px-4 py-8 text-center text-gray-500">
                                        <div className="flex flex-col items-center justify-center">
                                            ...
                                            <p className="text-lg font-medium text-gray-600">No sub-admins found</p>
                        ...
                        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
                            <div className="text-sm text-gray-700">
                                ...
                                <span className="font-medium">
                                    {Math.min(indexOfLastItem, subAdminList.length)}
                                </span>{" "}
                                of <span className="font-medium">{subAdminList.length}</span> results
                            </div>
